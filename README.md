...
`publish-game.yaml` boots the exported game before it packages anything, which is the only check that runs on the stripped template the game ships on rather than on an editor build. One target does it for all of them, since what it catches is decided when the template is compiled.
